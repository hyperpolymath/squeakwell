...
* Prove: recovery algorithm produces correct reconstructions when
sufficient data is available
* Prove: recovery reports failure (not corrupt data) when data is
insufficient
...
*MEDIUM* — Data recovery tool where incorrect recovery is worse than no
recovery. The "`never return corrupt data`" invariant is safety-critical
and should be proven.

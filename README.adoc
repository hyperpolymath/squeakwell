...
and VQL-UT's progressive type system to **reassemble damaged, fragmented, or
...
| Semantic types | Infer from Graph + Document + VQL-UT type checking
| Tensor data | Vector embeddings (expand dimensions), raw data re-ingest
|===

**Progressive typing is the ratchet.** Each VQL-UT level constrains the
...
* **VQL-UT integration** — Uses TypedQLiser as the type-checking layer.
  Each phase invokes progressively stricter VQL-UT levels.
